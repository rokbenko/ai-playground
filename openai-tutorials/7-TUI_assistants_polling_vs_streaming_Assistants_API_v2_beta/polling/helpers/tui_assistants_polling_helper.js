...
// Define an async function to run the assistant and periodically retrieve the run to check its status using the polling helper
async function runAssistantAndRetrieveRun(myThread) {
  try {
    // Run the assistant
    // Periodically retrieve the run to check its status using the polling helper
    const myRun = await client.beta.threads.runs.createAndPoll(
      // Polling helper
      (thread_id = myThread.id),
      {
      ...
      { pollIntervalMs: 5000 } // Poll assistant response every 5 seconds
    );

    return myRun;
  } catch (error) {
    // Handle error when running the assistant or retrieving the run
    if (error instanceof OpenAIError) {
      outputFormatter.red(
        `\nExiting the script...\nError running the assistant or retrieving the run:\n${error.message}\n\n`
      );
    } else {
      outputFormatter.red(
        `\nExiting the script...\nUnexpected error running the assistant or retrieving the run:\n${error.message}\n\n`
      );
    }
    process.exit(1);
  }
...
// Define an async function to retrieve the assistant answer
async function retrieveAssistantAnswer(myThread) {
  try {
    ...
    // Handle error when retrieving the assistant answer
    if (error instanceof OpenAIError) {
      outputFormatter.red(
        `\nExiting the script...\nError retrieving the assistant answer:\n${error.message}\n\n`
      );
    } else {
      outputFormatter.red(
        `\nExiting the script...\nUnexpected error retrieving the assistant answer:\n${error.message}\n\n`
      );
    }
    process.exit(1);
  }
